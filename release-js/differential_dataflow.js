...
(function( root, factory ) {
    if( typeof define === "function" && define.amd ) {
        ...
        Rust.differential_dataflow = factory();
    }
}( this, function() {
...
;
                Module.exports.register = function register(plan) {
...
;
    ...
    if( __load_asynchronously ) {
        return WebAssembly.instantiate( __wasm_module, __imports )
            .then( instance => {
                ...
                console.log( "Finished loading Rust wasm module 'differential_dataflow'" );
                return Module.exports;
            })
            .catch( error => {
                console.log( "Error loading Rust wasm module 'differential_dataflow':", error );
                throw error;
            });
    } else {
    ...
    if( typeof window === "undefined" && typeof process === "object" ) {
        ...
        const wasm_path = path.join( __dirname, "differential_dataflow.wasm" );
        const buffer = fs.readFileSync( wasm_path );
        const mod = new WebAssembly.Module( buffer );

        return __initialize( mod, false );
    } else {
        return fetch( "differential_dataflow.wasm" )
